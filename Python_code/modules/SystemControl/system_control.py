...
class system_control():
    ...
    def __init__(self,baro_params,data_buffer_size): #baro_params
        ...
        if (self.baro_scheme=="fixed_heart_rate"):
            ...
            """bring the activation function to this section"""
        ...
        if (self.baro_scheme == "simple_model"):
            ...
            self.delta_c_prime=0.0
            self.G_T = float(baroreflex.efferent.G_T.cdata)
            self.G_k1 = float(baroreflex.efferent.G_k1.cdata)
        ...
        if (self.baro_scheme=="Ursino_1998"):
            ...
            self.G_Ts=float(baroreflex.regulation_sym.G_Ts.cdata)
            self.D_Ts_in_second=float(baroreflex.regulation_sym.D_Ts.cdata)
            self.D_Ts = int(self.D_Ts_in_second / self.dt)
            self.tau_Ts=float(baroreflex.regulation_sym.tau_Ts.cdata)
            self.delta_Ts=0.0
            #contractility
            self.G_E = float(baroreflex.regulation_sym.G_elastance.cdata)
            self.G_L = float(baroreflex.regulation_sym.G_L.cdata)
            self.G_k3 = float(baroreflex.regulation_sym.G_k3.cdata)
            self.G_kcb = float(baroreflex.regulation_sym.G_kcb.cdata)
            self.G_k_force = float(baroreflex.regulation_sym.G_k_force.cdata)

            self.D_E_in_second=float(baroreflex.regulation_sym.D_elastance.cdata)
            self.D_E=int(self.D_E_in_second/self.dt)

            self.tau_E=float(baroreflex.regulation_sym.tau_elastance.cdata)
            self.tau_L = float(baroreflex.regulation_sym.tau_L.cdata)
            self.tau_k3 = float(baroreflex.regulation_sym.tau_k3.cdata)
            self.tau_kcb = float(baroreflex.regulation_sym.tau_kcb.cdata)
            self.tau_k_force = float(baroreflex.regulation_sym.tau_k_force.cdata)
            self.E_0=float(baroreflex.regulation_sym.initial_elastance.cdata)
            self.delta_E0=0.0
            #Parasympathetic (vagal) activity
            self.G_Tv=float(baroreflex.regulation_vagal.G_Tv.cdata)
            self.D_Tv_in_second=float(baroreflex.regulation_vagal.D_Tv.cdata)
            self.D_Tv = int(self.D_Tv_in_second / self.dt)
