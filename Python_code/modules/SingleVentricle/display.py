...
def display_growth(data_structure, output_file_string="", t_limits=[],dpi=None):
    ...
    ax3 = f.add_subplot(spec2[2, 0])
    ax3.plot('time','ventricle_wall_volume',data=data_structure)
    ax3.set_xlabel('time (s)')
    ax3.set_ylabel('wall_volume (liter)')

    ax4 = f.add_subplot(spec2[3, 0])
    ax4.plot('time','ventricle_wall_thickness',data=data_structure)
    ax4.set_xlabel('time (s)')
    ax4.set_ylabel('wall_thickness (m)')

    ax5 = f.add_subplot(spec2[4, 0])
    ax5.plot('time','ventricle_slack_volume',data=data_structure)
    ax5.set_xlabel('time (s)')
    ax5.set_ylabel('slack_volume (liter)')
